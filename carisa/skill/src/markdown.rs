//! Parsing of [`DomainSkill`] from markdown with YAML frontmatter.
//!
...
//! ```
//!
//! The frontmatter must be valid YAML and contain the required fields
//! `id`, `title`, and `description`. The body (instructions) must be
//! non-empty after trimming.

use crate::error::MarkdownError;
use crate::types::DomainSkill;
...
impl DomainSkill {
    ...
    /// YAML.
    ///
    /// Returns [`MarkdownError::MissingField`] if `id`, `title`, or
    /// `description` is missing or is not a YAML string.
    ///
    ...
    pub fn from_markdown(content: &str) -> Result<Self, MarkdownError> {
        ...
        serde_saphyr::from_str::<serde::de::IgnoredAny>(frontmatter).map_err(|e| {
            MarkdownError::YamlParse {
                id: scanned_id.clone(),
        ...
        let id = extract_field(frontmatter, "id").ok_or_else(|| MarkdownError::MissingField {
            id: scanned_id,
            title: scanned_title,
            field: "id".to_owned(),
        })?;
        ...
        let title =
            extract_field(frontmatter, "title").ok_or_else(|| MarkdownError::MissingField {
                id: Some(id.clone()),
                title: None,
                field: "title".to_owned(),
            })?;

        let description = extract_field(frontmatter, "description").ok_or_else(|| {
...
mod tests {
    ...
    fn yaml_parse_error_context(#[case] input: &str, #[case] expected_id: Option<&str>) {
    ...
    fn empty_or_whitespace_body(#[case] input: &str, #[case] expected_id: Option<&str>) {
        let err = DomainSkill::from_markdown(input).unwrap_err();
        match err {
    ...
    #[case::id_is_number("---\nid: 123\ntitle: T\ndescription: D\n---\nbody", "id")]
    #[case::title_is_bool("---\nid: x\ntitle: true\ndescription: D\n---\nbody", "title")]
    fn non_string_field_treated_as_missing(#[case] input: &str, #[case] expected_field: &str) {
        let err = DomainSkill::from_markdown(input).unwrap_err();
        match err {
